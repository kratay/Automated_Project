...
class Untitled <  Test::Unit::TestCase	  
	...
	@@loctPrdBoxQTY = "//input[@id='qty']"
	@@loctPrdBuyNow = "//div[@class='buyControls']/input[@value='Buy Now']"#"css=input.buy.submit"
	PD_BasketTitle = "Your Shopping Cart" #  Get Text for showing title
...
############ 1. Buy Now  #################################################################			
		...
		@seleniumnew.testClickBuyNow("Click at Buy Now then redirect to Basket page.", "PD-15,PD-38", 1, PD_BasketTitle);			
		...
		@seleniumnew.searchText($loctSrhBox, $loctSrhSubmit, productDetails[0]);	
		@seleniumnew.getSearchResult($loctSrhResult);
		@seleniumnew.gotoProductDetail($PD_xPrdGoto);		
		...
		@seleniumnew.testInCorrAddtoBasket(@@loctPrdBoxQTY, 
			@@loctPrdBuyNow,
			"", 
			"", 
			"Leave Qty box blank then click Buy Now.",
			"PD-13,PD-35",  
			false);	  		
		...
		@seleniumnew.testInCorrAddtoBasket(@@loctPrdBoxQTY, 
			...
			"", 
			"DD", 
			"Enter character in Qty box then click Buy Now.",
			"PD-14,PD-36",  
			false);					
...
############ 3. Out of Stock & Email Me  ############################					
		...
		self.writeResult("PD-END" , "Finsished to execute", "Done", "Finished")		
	end
